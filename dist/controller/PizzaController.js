...
const getPizza = async (req, res, next) => {
    await Pizza.find().then(response => {
        res.json({
            response
        });
    }).catch(err => {
...
const getPizzaById = (req, res, next) => {
    let pizzaId = req.body.pizzaId;
    Pizza.findById(pizzaId).then(response => {
        res.json({
            response
        });
    }).catch(err => {
...
const updatePizza = (req, res, next) => {
    ...
    Pizza.findByIdAndUpdate(pizzaId, { $set: update }, { new: true }).then(response => {
        if (!response) {
...
const deletePizza = (req, res, next) => {
    ...
    Pizza.findByIdAndDelete(pizzaId).then(response => {
        if (!response) {
            return res.status(404).json({ message: 'Pizza not found' });
        }
        res.json({
...
const searchPizza = (req, res, next) => {
    ...
    Pizza.find({ pizzaname: { $regex: query, $options: "i" } }).then(pizzas => {
        res.json(pizzas);
    }).catch(err => {
        res.json({
            message: "Error occurred"
